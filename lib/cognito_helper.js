...
module.exports.cognitoCreate = async (customer) => {
    try {
        if (customer) {
            const params = {
                UserPoolId: process.env.COGNITO_USER_POOL_ID,
...
module.exports.cognitoUpdate = async (updateData, emailAddress) => {
    ...
    try {
        ...
        const attributeList = []
        attributeList.push({ Name: 'custom:child_first_name', Value: updateData.child_first_name })
        attributeList.push({ Name: 'custom:country', Value: updateData.country })
        attributeList.push({ Name: 'custom:child_last_name', Value: updateData.child_first_name })
        attributeList.push({ Name: 'custom:child_age', Value: updateData.child_age })
        attributeList.push({ Name: 'custom:parent_first_name', Value: updateData.parent_first_name })
        attributeList.push({ Name: 'custom:parent_last_name', Value: updateData.parent_last_name })
        const cognitoParams = {
