    ...
    try {
        ...
        if (query && query.filter) {
            if (Array.isArray(query.filter)) {
                mergedFilter = Object.assign({}, ...query.filter)
            } else if (typeof query.filter === 'object') {
                mergedFilter = query.filter
            } else {
                // Handle invalid filter type
                reject(new Error('Invalid filter type. Filter must be an object or an array of objects.'))
            }
        }

        // Check if sorting by paddle_number is requested
        if (options.sort && options.sort.paddle_number) {
            const sortDirection = options.sort.paddle_number === 1 ? 1 : -1

            schema.find(mergedFilter).sort({ paddle_number: sortDirection }).exec().then((documents) => {
                // Paginate the sorted data
                schema.paginate(documents, options, (paginateError, paginatedDocuments) => {
                    if (paginateError) {
                        reject(paginateError)
                    }
                    resolve(paginatedDocuments)
                })
            })
                .catch((error) => {
                    reject(error)
            ...
            // Default sorting behavior if not sorting by paddle_number
            schema.paginate(mergedFilter, options, (error, documents) => {
                if (error) {
                    reject(error)
                }
                resolve(documents)
