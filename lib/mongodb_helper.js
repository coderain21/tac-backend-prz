...
module.exports.getAuctionprojection = async (document, Auction, projection) => {
    try {
        console.log('AAA', Auction, document)
        const query = {
            seller_email: document.seller_email, auction_id: document.auction_id, // Replace 'excluded_buyer_id' with the buyer_id you want to exclude
        ...
        console.log('auctions', query)
        const documents = await Auction.findOne(query, projection) // Await the query result
        return documents
    } catch (err) {
        return false
    }
...
module.exports.getBid = async (document, UniqueBid) => {
    try {
        const query = {
            seller_email: document.seller_email,
            auction_id: document.auction_id,
        ...
        const result = await UniqueBid.findOne(query)
        return result
    } catch (err) {
        console.log(err)
        return false
...
module.exports.updateArnStatus = async (document, status, StepFunctionArn) => {
    try {
        const query = {
            seller_email: document.seller_email,
            auction_id: document.auction_id,
        }
        const updateResult = await StepFunctionArn.updateMany(query, { $set: { status } })
        return updateResult
    } catch (error) {
        console.log('Error in updateArnStatus', error)
        return false
    }
}

module.exports.getArns = async (document, StepFunctionArn) => {
    try {
        const query = {
            seller_email: document.seller_email,
            auction_id: document.auction_id,
 ...
 *                            Returns false if there is an error during the process.
 */
module.exports.getAllExecutionArn = async (query, StepFunctionArn) => {
    try {
        // const query = {
...
module.exports.singleGetAllExecutionArn = async (auction, StepFunctionArn) => {
    try {
        const query = {
            auction_id: auction.auction_id,
            seller_email: auction.seller_email,
            lot_id: auction._id, // Replace 'excluded_buyer_id' with the buyer_id you want to exclude auction._id,
            status: 'RUNNING',
        }
...
module.exports.deleteBulk = async (Model, query) => {
    try {
        const result = Model.deleteMany(query)
        return result
    } catch (error) {
        console.log(error)
        return false
