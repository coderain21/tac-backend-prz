 ...
 */
module.exports.list = async (schema, query, options) => {
    try {
        console.log('##################', schema, query)

        // Convert paddle_number to a number for sorting
        if (options.sort_by === 'paddle_number') {
            options.sort = {
                $expr: {
                    $cond: {
                        if: { $isNumber: { $toInt: `$${options.sort_by}` } },
                        then: { $toInt: `$${options.sort_by}` },
                        else: 0,
                    },
                },
            }
            if (options.sort_order === 'desc') {
                options.sort = { $multiply: [-1, options.sort] }
            }
        }

        const documents = await schema.paginate(query, options)
        return documents
    } catch (error) {
