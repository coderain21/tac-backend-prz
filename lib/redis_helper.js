...
module.exports.createRedisClient = async () => {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            console.log(`Attempt ${attempt} to connect to Redis cluster...`)
            const cluster = new Redis.Cluster(
                ...
                ],
                {
                    ...
                    retryStrategy: (times) => {
                        ...
                        console.log(`Retry strategy called with times: ${times}, delay: ${delay}ms`)
                        return delay
                    },
                },
            )
            ...
            // await cluster.connect()
            console.log('Redis cluster client created successfully')
            return cluster
        } catch (err) {
            ...
            // Exponential backoff
            const backoffDelay = 2 ** attempt * baseDelay
...
module.exports.getClient = async () => {
    ...
    try {
        isConnecting = true
        redisClient = await module.exports.createRedisClient()
        return redisClient
    } finally {
        isConnecting = false
    }
...
async function findAndUpdateTime(lotInformation, client) {
    try {
        ...
        console.error('Failed to update lot time:', { error: err.message, lotId: lotInformation._id })
        throw err
    }
}
