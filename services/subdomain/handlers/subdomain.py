...
def update_app_client(userpoolid, client_id, client_name, subdomain, existing_domain_record):
    ...
    print('callback', callbackUrlArray)
    ...
    print('callback url after appending', callbackUrlArray)
    ...
    print('callbackUrl after removing', callbackUrlArray)
    ...
    print('response', response['UserPoolClient']['CallbackURLs'])
    return response
    ...
    try:
        ...
        for domain in existing_subdomains:
            ...
            if domain['prefix'] != os.environ['DEFAULT_SUB_DOMAIN'] and domain['prefix'] == existing_domain_record['subdomain']:
                print("insideeeeee ", domain, os.environ['DEFAULT_SUB_DOMAIN'] ,domain['prefix'] , existing_domain_record['subdomain'])
                remove_old = True
            else:
