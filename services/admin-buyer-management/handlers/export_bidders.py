...
def list_bidders(event, context):
    ...
    try:
        ...
        auction_id = ObjectId(event['pathParameters'].get(
            'auction_id', ''))
        print('auction_id', auction_id)
        seller_email = event['pathParameters'].get(
            'seller_email', '')
...
def export_as_csv(bidders, db):
    try:
        ...
        s3_key = f"admin/exports/bidders/{auction_id}/{filename}.csv"
