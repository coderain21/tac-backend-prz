'''This is hooks file for order-management service'''
...
def skip_404_test_results(transaction):
    if (
        ...
        '/subdomain' in transaction['request']['uri']
    ):
...
def set_authorization(transaction):
    ...
    if '/sales' in transaction['request']['uri'] or  '/seller' in transaction['request']['uri']:
