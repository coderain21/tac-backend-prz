"""This module is used to list the seller details """
import json
import os
import re
...
def seller_detail(event, context):
    """
    The `seller_detail` function retrieves a details corresponding to a seller based on the input query string.
    ...
    try:
        ...
        if seller_id is None:
            return {
                "statusCode": 404,
                "headers": headers,
                "body": json.dumps({"message": "Seller id not found"})
            }
        ...
        seller_detail = user_collection.find_one({"_id":seller_id},projection)
