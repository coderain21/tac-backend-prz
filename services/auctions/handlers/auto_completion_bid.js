...
const queueUrl = 'https://sqs.eu-west-2.amazonaws.com/929441721738/auto-completion-email-dev.fifo' // `${process.env.QUEUE_URL}`
...
module.exports.handler = async (event) => {
    try {
        ...
        for (const user of splittedUsers) {
            console.log('splittedUsers', user, user.buyer_id)
            const id = uuid.v4()
            params.Entries.push({
                Id: id,
                MessageBody: JSON.stringify(user),
                MessageDeduplicationId: uuid.v4(),
                MessageGroupId: uuid.v4(), // auctionData[0].seller_email,
            })
            // messageList.push(sqs.sendMessageBatch(params))
            // console.log('sqsResponse', send)
        }
        ...
        // const  sqsResponse = await Promise.all(messageList)
        return event
