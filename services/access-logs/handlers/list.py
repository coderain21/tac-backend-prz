...
def list_all_logs(event, context):
    try:
        # Extract email address from the request context
        email_address = event['requestContext']['authorizer']['claims'].get('cognito:username')
        ...
        print('sort_by', sort_by, 'sort_order', sort_order, 'page', page, 'limit', limit, 'start_date', start_date, 'end_date', end_date, 'export', export)
        ...
        total_records = access_log_collection.count_documents(query)
        total_pages = math.ceil(total_records / limit)

        if not access_log_list:
            print('No access logs found matching the criteria')

        # Prepare the response body
        body = {
            "data": access_log_list,
        ...
        if export:
            body["download_link"] = export_as_csv(access_log_list, email_address)
