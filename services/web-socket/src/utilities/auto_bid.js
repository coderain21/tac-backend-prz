    ...
    try {
        ...
        if (all_bidders.length > 0) {
            ...
            if (highestBidder.base_price > record.max_bid) {
                ...
                const otherUsers = all_bidders.filter((bid) => bid.buyer_id === latestRecordId.buyer_id)
                if (otherUser) {
                    maximumBid = latestRecordId.max_bid
                    const updateCurrentBid = await updateCurrentBidAmounts(otherUsers, userAmount, client, maximumBid)
                }
            } else {
                let price = highestBidder.max_bid
                if (all_bidders.length > 0) {
                    price = highestBidder.base_price === undefined ? highestBidder.max_bid : highestBidder.base_price
                }
