...
resource "aws_appautoscaling_policy" "ecs_cpu_scaling" {
  name               = "ecs-cpu-scaling-${var.STAGE}"
...
resource "aws_appautoscaling_policy" "ecs_memory_scaling" {
  name               = "ecs-memory-scaling-${var.STAGE}"
  policy_type        = "StepScaling"
  resource_id        = aws_appautoscaling_target.target.resource_id
  scalable_dimension = aws_appautoscaling_target.target.scalable_dimension
  service_namespace  = aws_appautoscaling_target.target.service_namespace
...
resource "aws_appautoscaling_policy" "ecs_request_scaling" {
  name               = "ecs-request-scaling-${var.STAGE}"
  policy_type        = "StepScaling"
  resource_id        = aws_appautoscaling_target.target.resource_id
  scalable_dimension = aws_appautoscaling_target.target.scalable_dimension
  service_namespace  = aws_appautoscaling_target.target.service_namespace
...
# CloudWatch Alarm for ECS Memory Scale Out
...
# CloudWatch Alarm for ECS Memory Scale In
...
resource "aws_cloudwatch_metric_alarm" "ecs_requests_scale_out" {
  ...
  statistic           = "Maximum"
  threshold           = "100"
  alarm_description   = "Scale out when requests > 500 for 2 periods"
