...
run_command terraform -chdir=devops/ecs_new_service init -backend-config="bucket=${log_bucket}" -backend-config="key=$STAGE/devops/ecs_new/terraform.tfstate" -backend-config="profile=${PROFILE_MAIN}"
run_command terraform -chdir=devops/ecs_new_service apply -auto-approve
