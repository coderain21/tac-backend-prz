...
data "external" "token" {
  program = ["bash", "-c", "echo \"{\\\"token\\\":\\\"$(curl -s -X POST -u 'F6BUkzxemBHrQRNmBw:bjuamEZ7jFm5ms43k49NBSCK4n6zTUy2' https://bitbucket.org/site/oauth2/access_token -d grant_type=client_credentials -d code=420 | jq -r '.access_token')\\\"}\""]
}
